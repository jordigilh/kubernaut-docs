...
By default, `WorkflowExecution.Spec.ClusterID` is copied verbatim from `RemediationRequest.Spec.ClusterID` -- the execution cluster matches the signal's origin cluster. A `RemediationWorkflow` can override this by declaring `spec.execution.clusterId` ([DD-FLEET-008](https://github.com/jordigilh/kubernaut/blob/main/docs/architecture/decisions/DD-FLEET-008-workflow-declared-execution-cluster.md), BR-FLEET-004): when set, it takes precedence over the RR's `ClusterID` for the resulting `WorkflowExecution.Spec.ClusterID`, letting a workflow run on a cluster other than the one that reported the signal -- for example a GitOps-hub cluster that owns the reconciling ArgoCD/Flux instance, or an aggregator cluster with reach to a resource-constrained edge device that can't run a Job itself. A workflow-declared `clusterId` is subject to the same fail-closed dispatch behavior as an RR-supplied `ClusterID`: it must resolve to a cluster already registered with the fleet MCP Gateway, or dispatch fails. See [Per-Workflow Execution Cluster](../user-guide/workflow-authoring.md#per-workflow-execution-cluster) for the field reference and use-case detail.
