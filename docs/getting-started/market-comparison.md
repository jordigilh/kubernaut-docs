...
Kubernaut is **fully self-hosted**. The entire platform -- Gateway, signal processing, AI investigation, remediation orchestration, workflow execution, data storage, audit pipeline -- runs inside the customer's Kubernetes cluster. Investigation transcripts, remediation records, audit events, and telemetry are stored locally in PostgreSQL and never leave the cluster. When using a cloud LLM provider (OpenAI, Anthropic, Gemini, Vertex AI, etc.), investigation prompts are sent to the provider's API; for full data containment, use a [locally hosted LLM](../operations/disconnected-install.md) (Ollama, vLLM, or any OpenAI-compatible endpoint).
...
- **No vendor lock-in**: Works with OpenAI, Anthropic, Google Gemini, Vertex AI (Claude or Gemini models), and any OpenAI-compatible endpoint -- which covers Azure OpenAI, Ollama, vLLM, and LlamaStack (native AWS Bedrock support is not yet shipped, tracked in [#1582](https://github.com/jordigilh/kubernaut/issues/1582)) -- plus any monitoring stack (Prometheus, Alertmanager, any webhook-capable system), and runs fully self-hosted.
