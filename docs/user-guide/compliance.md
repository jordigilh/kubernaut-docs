...
- **11 services** emit structured audit events covering the full remediation lifecycle, from signal ingestion through effectiveness assessment. Every phase transition, routing decision, approval, execution, and notification is recorded ([DD-AUDIT-003]).
- The **workflow discovery audit trail** records every step of the LLM's 3-step selection protocol: which action types were listed, which workflows were scored, which workflow was retrieved, and whether the selection passed validation ([BR-AUDIT-021-030]). As of v1.6 (DD-WORKFLOW-019), these events (`workflow.catalog.actions_listed`, `workflows_listed`, `workflow_retrieved`, `selection_validated`) are generated by **Kubernaut Agent**, not DataStorage — the event type strings are unchanged, only the emitting service moved.
...
- Workflow catalog changes (create, update, delete) are captured by a single event generator: the **Auth Webhook**, which admits every `RemediationWorkflow`/`ActionType` CRD change and records the operator identity alongside the change itself (`remediationworkflow.admitted.create/delete`, `actiontype.admitted.create/update/delete`, per [DD-WEBHOOK-003]).
- Action type taxonomy changes follow the same Auth-Webhook-only pattern.

!!! note "v1.6: no more DataStorage-side catalog audit"
    Through v1.5, DataStorage independently audited catalog state changes alongside the Auth Webhook's operator-attribution event (a dual-event pattern). As of v1.6 (DD-WORKFLOW-018), the `remediation_workflow_catalog`/`action_type_taxonomy` Postgres tables were dropped — the `RemediationWorkflow`/`ActionType` CRDs (etcd) are the sole source of truth, admitted directly by the Auth Webhook with zero DataStorage round-trips. DataStorage's production code today emits exactly one audit event type (`datastorage.ratelimit.denied`, per-IP rate limiting); it no longer independently audits catalog changes.
...
Kubernaut defines auditable events per service in [DD-AUDIT-003]. Nine services have mandatory (P0) audit requirements (Gateway, Data Storage, Auth Webhook, AI Analysis, Workflow Execution, Notification, Effectiveness Monitor, API Frontend, and Kubernaut Agent); two have recommended (P1) (Signal Processing, Remediation Orchestrator) — 11 of 14 services in total. Event types are hierarchically named (`service.category.action`) and documented in the [audit event catalog](../architecture/audit-pipeline.md#emitting-services).
...
| **CC7.2** Monitoring | 11 services, full lifecycle audit, workflow discovery trail, Prometheus metrics | [DD-AUDIT-003], [BR-AUDIT-021-030] |
...
| **AU-2** Auditable Events | Defined per service (9 mandatory, 2 recommended) | [DD-AUDIT-003] |
