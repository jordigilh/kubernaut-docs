...
| `apifrontend.config.rbac.consoleAccessGroups` | OIDC groups granted the coarse-grained `kubernaut.ai/console` "use" gate (v1.5.6, #1919). Checked in addition to the existing per-tool `kubernaut.ai/tools` grant on every `/mcp` and `/a2a/invoke` call -- but only once `consoleAccessAuthorizationCheckEnabled` (below) is set to `true`. | All 6 built-in persona names: `sre`, `ai-orchestrator`, `cicd`, `observability`, `l3-audit`, `remediation-approver` |
| `apifrontend.config.rbac.consoleAccessAuthorizationCheckEnabled` | Enables the authorization check on the console gate above, so `consoleAccessGroups` actually takes effect (#2148/#2150). When `false`, the gate is authentication-only -- any authenticated user passes, regardless of `consoleAccessGroups`. Per-tool authorization is unaffected either way. | `false` |
...
!!! danger "Upgrade action required for custom persona groups (v1.5.6)"
    ...
    This only applies once `consoleAccessAuthorizationCheckEnabled` is set to `true` -- it defaults to `false`, so a fresh install (or one that hasn't opted in yet) is authentication-only and unaffected by `consoleAccessGroups` misconfiguration. See [Security & RBAC: `consoleAccessAuthorizationCheckEnabled`](../architecture/security-rbac.md#console-access-authorization-check-enabled).
